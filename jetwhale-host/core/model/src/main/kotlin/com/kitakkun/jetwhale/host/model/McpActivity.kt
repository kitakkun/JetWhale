 ...
 *
 * @property arguments Every argument the call was made with, including `pluginId` and `sessionId`.
 * @property response What the call produced, rendered to a string and shortened to
 * [MAX_RESPONSE_LENGTH]. Carries the failure message when the call failed, and is empty when there
 * is nothing to show.
 */
 ...
 *
 * @property startedCount Monotonically increasing count of tool calls started. A tool call can
 * begin and end faster than the UI samples [runningInvocations], so that list is often empty
 * between rapid calls and can't be sampled reliably. Watching this counter change instead never
 * misses a call, because the value differs across frames even when individual increments are
 * skipped.
 * @property lastStartedInvocation The most recently started tool call, kept so the UI can name and
 * attribute it.
 * @property recentCalls Tool calls that have already completed, newest first, capped at
 * [MAX_RECENT_CALLS]. This is a live troubleshooting aid rather than an audit log, so the oldest
