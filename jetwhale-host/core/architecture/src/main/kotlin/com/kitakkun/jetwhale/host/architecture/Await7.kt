...
@PublishedApi
internal data class Septuple<out A, out B, out C, out D, out E, out F, out G>(
    val first: A,
    val second: B,
    val third: C,
    val fourth: D,
...
@PublishedApi
internal inline fun <T1, T2, T3, T4, T5, T6, T7, R> Reply.Companion.combine(
    r1: Reply<T1>,
    r2: Reply<T2>,
    r3: Reply<T3>,
    r4: Reply<T4>,
    ...
    r1.isNone || r2.isNone || r3.isNone || r4.isNone || r5.isNone || r6.isNone || r7.isNone -> none()
...
@Composable
inline fun <T1, T2, T3, T4, T5, T6, T7> Await(
    state1: DataModel<T1>,
    state2: DataModel<T2>,
    state3: DataModel<T3>,
    state4: DataModel<T4>,
    ...
    crossinline content: @Composable (T1, T2, T3, T4, T5, T6, T7) -> Unit,
) {
    ...
    LaunchedEffect(id, state1, state2, state3, state4, state5, state6, state7) {
        host[id] = listOf(state1, state2, state3, state4, state5, state6, state7).any { it.isAwaited() }
    }
    DisposableEffect(id) {
        onDispose {
            host.remove(id)
