            ...
            pluginScenes.getOrPut("$pluginId:$sessionId") {
                ...
                composeScene.setContent {
                    pluginBridgeProvider.PluginEntryPoint {
                        // Headless plugins (not a JetWhaleHostPluginUi) render no content. The plugin
                        // talks to the agent through its own messenger, so there is no context here.
                        (pluginInstance as? JetWhaleHostPluginUi)?.Content()
                    }
                }
