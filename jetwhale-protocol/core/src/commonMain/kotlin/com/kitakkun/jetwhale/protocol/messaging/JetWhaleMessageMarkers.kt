...
/**
 * Marker for fire-and-forget messages. Only types implementing this can be passed to a messenger's
 * `send`; handlers are registered with `onEvent`. A reply type implements
 ...
 * to a messenger's `request`; handlers are registered with `onRequest`. Declaring the reply
