...
internal class ReplaceBackStackCommand(
    ...
    override val description =
        "Puts the app into an exact navigation state by replacing the whole back stack with the given keys, root first. Use it to " +
            "reproduce a deep-linked state in one step instead of pushing screen by screen."

    private val keys by jsonArray(
        "The new back stack as a JSON array of NavKey objects, root first, e.g. " +
            "[{\"type\":\"Home\"},{\"type\":\"Detail\",\"id\":\"42\"}]. Must not be empty.",
    )
