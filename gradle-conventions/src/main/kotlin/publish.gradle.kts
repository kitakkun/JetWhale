 ...
 * name (`network`, `nav3`, `semantics`, `storage`, `actions`), `gradle-plugin` and `agent-plugin`.
 */
private fun String.selects(artifactId: String): Boolean = when (this) {
    ...
    "sdk" -> listOf("network", "nav3", "semantics", "storage", "actions", "gradle-plugin", "agent-plugin").none { it.selects(artifactId) }
    else -> false
}
