 ...
 * name (`network`, `nav3`, `semantics`, `storage`, `coroutines`), `gradle-plugin` and `agent-plugin`.
 */
private fun String.selects(artifactId: String): Boolean = when (this) {
    ...
    "coroutines" -> artifactId.startsWith("jetwhale-coroutine-inspector")
    ...
    "sdk" -> listOf("network", "nav3", "semantics", "storage", "coroutines", "gradle-plugin", "agent-plugin").none { it.selects(artifactId) }
    else -> false
}
