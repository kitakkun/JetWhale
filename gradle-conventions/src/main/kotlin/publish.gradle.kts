 ...
 * name (`network`, `nav3`, `semantics`, `storage`, `mainthread`), `gradle-plugin` and `agent-plugin`.
 */
private fun String.selects(artifactId: String): Boolean = when (this) {
    ...
    "mainthread" -> artifactId.startsWith("jetwhale-main-thread-monitor")
    ...
    "sdk" -> listOf("network", "nav3", "semantics", "storage", "mainthread", "gradle-plugin", "agent-plugin").none { it.selects(artifactId) }
    else -> false
}
