 ...
 * name (`network`, `nav3`, `semantics`, `storage`, `permissions`), `gradle-plugin` and `agent-plugin`.
 */
private fun String.selects(artifactId: String): Boolean = when (this) {
    ...
    "permissions" -> artifactId.startsWith("jetwhale-permissions")
    ...
    "sdk" -> listOf("network", "nav3", "semantics", "storage", "permissions", "gradle-plugin", "agent-plugin").none { it.selects(artifactId) }
    else -> false
}
