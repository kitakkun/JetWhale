 ...
 * name (`network`, `nav3`, `semantics`, `storage`, `deeplinks`), `gradle-plugin` and `agent-plugin`.
 */
private fun String.selects(artifactId: String): Boolean = when (this) {
    ...
    "deeplinks" -> artifactId.startsWith("jetwhale-deep-links")
    ...
    "sdk" -> listOf("network", "nav3", "semantics", "storage", "deeplinks", "gradle-plugin", "agent-plugin").none { it.selects(artifactId) }
    else -> false
}
