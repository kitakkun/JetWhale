 ...
 * name (`network`, `nav3`, `semantics`, `storage`, `background`), `gradle-plugin` and `agent-plugin`.
 */
private fun String.selects(artifactId: String): Boolean = when (this) {
    ...
    "background" -> artifactId.startsWith("jetwhale-background-work")
    ...
    "sdk" -> listOf("network", "nav3", "semantics", "storage", "background", "gradle-plugin", "agent-plugin").none { it.selects(artifactId) }
    else -> false
}
